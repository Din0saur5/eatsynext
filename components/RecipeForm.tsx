...
const RecipeForm = ({ recipe = defaultRecipe }: RecipeFormProps) => {
  ...
  const handleAddIngredient = () => {
    setFormData({ ...formData, ingredient_list: [...formData.ingredient_list, newIngredient] });
    setNewIngredient({ food: '', quantity: 0, unit: '', text: '' }); // Reset the new ingredient input
  };

  const handleRemoveIngredient = (index: number) => {
    const updatedIngredients = formData.ingredient_list.filter((_, idx) => idx !== index) as Ingredient[];
    ...
    <div>
      <form onSubmit={onSubmit}>
        ...
        <input type="radio" name="my-accordion-2" defaultChecked /> 
                <div className="collapse-title text-xl font-medium">Basic Recipe Info</div>
                <div className="collapse-content">
                <input className='rounded-lg mb-2 mr-1' type="text" name="name" value={formData.name} onChange={(e)=>onChange(e)} placeholder="Name"  required/>
                <div>
                  <input className='rounded-lg mb-2 mr-1' type="text" name="image" value={formData.image ?? ''} onChange={(e)=>onChange(e)} placeholder="Image URL"  required/>
                  <button className="btn" onClick={()=>{
                     const modal = document.getElementById('previewImg') as HTMLDialogElement | null;
                     if (formData.image === ''){
                ...
                <input className='rounded-lg mb-2 mr-1' type="number" name="time" value={formData.time ?? 0} onChange={(e)=>onChange(e)} placeholder="Est. Cook Time" required />
                <AutoCompleteInput 
                      dataSet={dishTypes}
                      onSuggestionSelect={(value) => setFormData({ ...formData, dish_type: value })}
                      onInputChange={(e) => setFormData({ ...formData, dish_type: e.target.value})}
                ...
                <AutoCompleteInput 
                  ...
                  onSuggestionSelect={(value) => setFormData({ ...formData, cuisine: value })}
                  onInputChange={(e) => setFormData({ ...formData, cuisine: e.target.value})}
                  inputClassName="border border-gray-400 rounded-lg mr-1 p-2"
                  suggestionsClassName=""
                >
                  <input
                ...
                <select name="meal_type" value={formData.meal_type ?? ''} onChange={(e)=>onChange(e)}  required> 
          ...
          <div className="collapse collapse-arrow bg-base-200">
            <input type="radio" name="my-accordion-2" />
                <div className="collapse-title text-xl font-medium">Description and Tags?</div>
                <div className="collapse-content">
                    <div className='flex flex-col'>
                <textarea className='rounded-lg mb-2 mr-1' name="description" value={formData.description ?? ''} onChange={(e)=>onChange(e)} placeholder="Description"  required/>
                <label className='text-gray-500' htmlFor='tags'>add up to 5 tags for better searchability </label>
                <input 
        ...
        <div>
          ...
          <div className="collapse collapse-arrow bg-base-200">
            ...
            <div className="collapse-content">
              {/* New ingredient inputs */}
                <input type="text" name="food" value={newIngredient.food} onChange={handleNewIngredientChange} placeholder="Food"  />
                <input type="number" name="quantity" value={newIngredient.quantity.toString()} onChange={handleNewIngredientChange} placeholder="Quantity" />
                <AutoCompleteInput 
                      dataSet={cookingMeasurementUnits}
                      onSuggestionSelect={(value) => setNewIngredient({ ...newIngredient, unit: value })}
                      onInputChange={(e)=>handleNewIngredientChange(e)} 
                      inputClassName="border border-gray-400 rounded-lg mr-1 p-2"
                      suggestionsClassName=""
                    >
                   <input type="text" name="unit" value={newIngredient.unit} placeholder="Unit"  />

                </AutoCompleteInput>
                
                
                <input type="text" name="text" value={newIngredient.text} onChange={handleNewIngredientChange} placeholder="Extra Details"  />
                ...
                {(formData.ingredient_list ?? []).map((ingredient, index) => {
                  ...
                  return (
                    <div key={index}>
                      {`${ing.quantity ?? ''} ${ing.unit ?? ''} ${ing.food ?? ''}: ${ing.text ?? ''}`}
          ...
          <div className="collapse collapse-arrow bg-base-200">
            <input type="radio" name="my-accordion-2" />
                ...
                <div className={formValidationError? 'tooltip tooltip-bottom':  ''} data-tip={`${formValidationError}`}>
                <button  disabled={!!formValidationError} className={`btn mt-4 px-4 py-2 ${formValidationError ? 'bg-gray-500 text-gray-200' : ''}` } type="submit">Create Recipe</button>
                </div>
                </div>
